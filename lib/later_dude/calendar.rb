...
module LaterDude
  ...
  class Calendar
    ...
    def initialize(year, month, options={}, &block)
      ...
      if (hex[0].hex > 102 && hex[1].hex > 102) || (hex[1].hex > 102 && hex[2].hex > 102) || (hex[0].hex > 102 && hex[2].hex > 102) || (hex[0].hex == 102 && hex[1].hex == 255) || (@header_color != "#990066")
        @text_color = '#000000'
      else
        @text_color = '#ffffff'
      end
    ...
    def to_html
      content_tag(:div, :class => "calendar") do
        content_tag(:div, "#{show_month_names} #{@year}".html_safe, :class => 'header', :style => "background-color:#{@header_color};color:#{@text_color};") +
